...
namespace LaTeXAPI.Class
{
    ...
    public class MessageCode : ILogin, IMessageCode
    {
        ...
        /// <returns></returns>
        public AliyunMessgeRespons SendVerifyCode(string tel, string templatecode)
        {
            if (!IsPhoneNumber(tel))
            {
                throw new Exception(MyException.TelFormatWrong(tel));
            ...
            AliyunMessgeRespons aliyunMessageResult = JsonConvert.DeserializeObject<AliyunMessgeRespons>(_message.Send(tel, SignName, TemplateCode, dic_code));
