...
namespace LaTeXAPI.Class
{
    public class RoleManage : IPermission
    {
        ...
        {
            //1. select client_id,role_id from client_role where client_id=@client_id   得到role_id集合
            //2. select permission_id from role_permission where role_id=@role_id   得到permission_id集合
            //3. select * from permission where id=@id    得到permission集合
